...
============================================================
Utilities to build a PyTorch Geometric hypergraph dataset from raw
text files and persist it in `InMemoryDataset` format.

Includes:

1) Raw → PyG `Data` construction (`load_dataset`)
   - Reads REQUIRED edge labels from:    edge-labels-<dataset>.txt (CSV rows per hyperedge)
   - Reads node embeddings from a word2vec-style file:
       first line: "<n> <dim>"
       subsequent lines: "<id> <d1> ... <d_dim>"
     A feature matrix of shape (num_node, dim) is preallocated; rows are
     filled by the IDs present in the embedding file. Unseen IDs remain
     as random values.
   - Parses hyperedges from:             hyperedges-<dataset>.txt
       (one comma-separated node ID list per line)
   - Builds a star-expansion/bipartite incidence graph with directed pairs
     [[nodes→hyperedges], [hyperedges→nodes]] and then coalesces/sorts
     duplicates via `torch_sparse.coalesce`.
   - Normalizes node IDs so the minimum node ID becomes 0.
   - Sets meta fields on `Data`:
       - `n_x`              : number of (shifted) nodes
       - `num_hyperedges`   : count of hyperedges

2) Serialization helper (`save_data_to_pickle`)
   - Pickles any Python object (e.g., `Data`) to `<root>/<name>`.

3) PyG dataset wrapper (`dataset_Hypergraph`)
   - Directory layout: `<root>/<name>/{raw,processed}`
   - Supported names:   mimic3 | cradle | promote
   - `download()`: calls `load_dataset(...)` and writes a raw pickle
                    at `<root>/<name>/raw/<name>`
...
- edge-labels-<dataset>.txt      # REQUIRED; CSV per hyperedge (no header)
...
- node-embeddings-<dataset>      # first line: "<n> <dim>", then "<id> <d1> ... <d_dim>"
                                # path is passed via `node_feature_path`
"""
