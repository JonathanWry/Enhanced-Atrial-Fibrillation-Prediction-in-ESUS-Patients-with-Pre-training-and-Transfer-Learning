        ...
        **kwargs,
        ...
        if (self.step % self.update_weights_every) == 0:
            ...
            for i, loss in enumerate(losses):
                ...
                grad = torch.cat([
                    torch.flatten(grad).unsqueeze(0) if grad is not None and grad.dim() == 0
                    else torch.flatten(grad) if grad is not None
                    else torch.zeros_like(shared_parameters[j]).flatten()  # Use shared_parameters[j] to match shape
                    for j, grad in enumerate(g)
                ])
                grads[i] = grad
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for i, loss in enumerate(losses):
            ...
            g = [torch.flatten(grad) if grad is not None else torch.zeros_like(param) for grad, param in zip(g, params)]
            grads[i] = g
...
class PCGrad(WeightMethod):
    ...
    def backward(
        ...
        **kwargs,
        ...
        return weighted_loss, {}  # NOTE: to align with all other weight methods
...
class CAGrad(WeightMethod):
    ...
    def backward(
        ...
        **kwargs,
        ...
        weighted_loss = self.get_weighted_loss(losses, shared_parameters)
        return weighted_loss, {}  # NOTE: to align with all other weight methods
...
class DynamicWeightAverage(WeightMethod):
    ...
    def __init__(
        self, n_tasks, device: torch.device, iteration_window: int = 8, temp=2.0
    ):
        """
    ...
    def get_weighted_loss(self, losses, **kwargs):
        ...
        cost = np.array([loss.detach().cpu().numpy() for loss in losses])
        ...
        loss = (task_weights * losses_tensor).mean()

        self.running_iterations += 1

        return loss, dict(weights=task_weights)

class GradNorm(WeightMethod):
    def __init__(
        self,
        n_tasks: int,
        device: torch.device,
        ...
        self.l0 = None  # Initial task losses for normalization
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        if self.l0 is None:
            self.l0 = [loss.detach().clone() for loss in losses]  # Store initial loss values for each task
        losses_tensor = torch.stack(losses)
        ...
        for i, loss in enumerate(losses):
            ...
            grad_norm = torch.cat([g.view(-1) for g in grad if g is not None]).norm(2)
            grads.append(grad_norm)

        grads = torch.stack(grads)

        # Compute loss ratios
        loss_ratio = torch.tensor([loss.detach() / l0 for loss, l0 in zip(losses, self.l0)], device=self.device)
        relative_rate = loss_ratio / loss_ratio.mean()

        # Compute GradNorm loss
        gw_avg = grads.mean().detach()
        target_norms = (gw_avg * relative_rate**self.alpha).detach()
        gradnorm_loss = (torch.abs(grads - target_norms)).sum()

        # Update the task weights using gradient descent on the GradNorm loss
        gradnorm_loss.backward(retain_graph=True)
        with torch.no_grad():
            new_weights = self.weights - self.lr * self.weights.grad
            new_weights = torch.clamp(new_weights, min=1e-3)  # Ensure weights are positive
            new_weights /= new_weights.sum()
            self.weights.data.copy_(new_weights)

        # Clear gradients for the next update
        self.weights.grad = None

        return weighted_loss, {"weights": self.weights.detach().cpu().numpy()}
